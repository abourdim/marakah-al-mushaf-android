#!/usr/bin/env bash
# ============================================================================
#  Marakah al-Mushaf Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/35-marakah-al-mushaf}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.marakahalmushaf}"
AVD_NAME="${AVD_NAME:-marakah_al_mushaf_test}"
...
cmd_test() {
  ...
  local out="/tmp/marakah-al-mushaf-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Marakah al-Mushaf Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
